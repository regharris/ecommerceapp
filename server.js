...
const database = `SELECT products.productName, products.productDescription, productCategory,products.productImage, price.price FROM products INNER JOIN price ON products.Id = price.price_id`;
...
app.get("/api/productfilter/:query", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM products WHERE productCategory LIKE ?",
    [category],
    function(err, result) {
      if (err) throw err;
      console.log(result);
...
app.get("/api/pricefilter/:query", (req, res) => {
  ...
  connection.query(
    "SELECT products.*, price.price FROM products INNER JOIN price ON products.Id = price.product_id WHERE price.price < ?;",
    [price],
    function(err, result) {
      if (err) throw err;
      console.log(result);
